package com.flightcontroller.mavlink;
...
public abstract class MavLinkConnection {
	...
	public void sendMavPacket(MAVLinkPacket packet) {
		if (!packetsToSend_.offer(packet)) {
			LogManager.INSTANCE.addEntry("Sending queue overfilled: possible poor connection",
                    LogSeverity.WARNING);
		}
	}
	...
	private void reportConnect() {
		LogManager.INSTANCE.addEntry("Connection to radio has been established", LogSeverity.INFO);
		synchronized (listeners_) {
			for (MavLinkConnectionListener listener : listeners_) 
				listener.onConnect();
		}
	...
	private void reportDisconnect() {
		LogManager.INSTANCE.addEntry("Connection to radio has been lost", LogSeverity.INFO);
		synchronized (listeners_) {
			for (MavLinkConnectionListener listener : listeners_) 
